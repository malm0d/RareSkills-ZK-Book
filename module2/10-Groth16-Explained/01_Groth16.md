...
### Verifier.sol::Verifier::verifyingkey (L173-186)
...
The `verifyingkey` function is used to return the hardcoded values of the following constants:
- `vk.alfa1` $\rightarrow [\alpha]_1$
- `vk.beta2` $\rightarrow  [\beta]_2$
- `vk.gamma2` $\rightarrow [\gamma]_2$
...
```math
\begin{align*}
[\Psi_i]_1 &= [\frac{\Psi_i}{\gamma}]_1 = \frac{(\alpha v_i(\tau) + \beta u_i(\tau) + w_i(\tau))G_1}{\gamma}
\end{align*}
```

And thus, `vk.IC[i]` is equals to:
